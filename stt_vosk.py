...
+ integrazione con EVA (server Flask su http://127.0.0.1:5000/json).

Dipendenze di sistema (Raspberry Pi / Debian-like / Ubuntu):
    sudo apt update
...
DEFAULT_CFG = {
    "lang": "it",
    # Sample rate di default: pu� essere sovrascritto da vosk.json o da --samplerate
    "samplerate": 48000,
    "audio_device": "auto",
    "show_partial": False,
    "models_dir": "./models",
    ...
    "tts": {
        ...
        # Dispositivo ALSA per l'uscita audio (es. "plughw:1,0"); pu� essere lasciato vuoto
        "output_device": "plughw:1,0"
    },
    # Config per la chiamata a EVA (server Flask)
    "eva": {
        "enabled": True,
...
def ensure_config(path: str) -> dict:
    ...
    try:
        ...
        # Se il file � in una codifica strana, lo rinominiamo e lo ricreiamo
        backup = path + ".bak"
...
class PiperTTS:
    ...
    def speak(self, text: str, wav_path: str = "/tmp/robot_speech.wav"):
        ...
        # Riproduci WAV con SoX, usando eventualmente un device ALSA specifico
        env = os.environ.copy()
        ...
        try:
            ...
            print(f"[PIPER] Errore durante la riproduzione con SoX: {e}")
...
def pick_input_device(device_arg) -> int | None:
    ...
    if device_arg is None or str(device_arg).lower() == "auto":
        # Se trovi un ReSpeaker lo usi, altrimenti il default di sistema
...
# === integrazione con EVA ===
def ask_eva(text: str, eva_cfg: dict) -> str:
    """
    Invia il testo a EVA (Flask + Ollama) e restituisce la risposta in chiaro.
    """
    if not eva_cfg.get("enabled", True):
        return ""

    url = eva_cfg.get("url", "http://127.0.0.1:5000/json")
    payload = {
        "query": text
    }

    model = (eva_cfg.get("model") or "").strip()
    profile = (eva_cfg.get("profile") or "").strip()
    if model:
        payload["model"] = model
    if profile:
    ...
    try:
        ...
        print(f"[EVA] Errore chiamando EVA su {url}: {e}")
        return "Si � verificato un errore parlando con EVA."


def main():
    ap = argparse.ArgumentParser(description="Ascolto microfono + STT con Vosk + wake-word + Piper TTS + EVA.")
    ...
    if tts_cfg.get("enabled", True):
        try:
            piper = PiperTTS(
                models_dir=tts_cfg.get("models_dir", "./models/piper"),
            ...
            while True:
                ...
                if recognizer.AcceptWaveform(data):
                    ...
                    if wake_cfg.get("enabled", True):
                        ...
                        if not triggered:
                            # Nessuna wake-word: ignoro la frase completa
                            continue
                            ...
                            if piper is not None:
                                ...
                                if ack:
                                    try:
                                        ...
                                        print(f"[PIPER] Errore speak (ack): {e}")

                            # Chiamata a EVA
                            if eva_cfg.get("enabled", True):
                                answer = ask_eva(corrected, eva_cfg)
                                print(f"[EVA] Risposta: {answer!r}")
                                if piper is not None and answer:
                                    try:
                                        piper.speak(answer)
                                    except Exception as e:
                                        print(f"[PIPER] Errore speak (EVA): {e}")

                            # Svuota la coda audio per evitare overflow mentre TTS/EVA hanno lavorato
                        ...
                        # Wake-word disabilitata: post-processo direttamente il testo
                        do_beep(beep_cfg)
