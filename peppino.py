...
FASTER_WHISPER_CFG = ASR_CFG.get("faster-whisper", {}) or {}
...
def _load_faster_whisper_model():
    ...
    model_name = FASTER_WHISPER_CFG.get("model", "tiny")
    model_path = FASTER_WHISPER_CFG.get("model_path")
    compute_type = FASTER_WHISPER_CFG.get("compute_type", "int8")

    # === LA MODIFICA FONDAMENTALE ===
    # Forza l'uso della CPU per bypassare i problemi del driver NVIDIA
    device = "cpu"
    # ================================

    if model_path:
        _faster_whisper_model = WhisperModel(model_path, device=device, compute_type=compute_type)
    else:
...
# =============== Main ===============
def main():
    print(f"[INFO] Avvio Telegram bridge v.1.02 → {APP_BASE_URL} | default model: {DEFAULT_MODEL}", file=sys.stderr)
    if ALLOWED_CHAT_IDS:
